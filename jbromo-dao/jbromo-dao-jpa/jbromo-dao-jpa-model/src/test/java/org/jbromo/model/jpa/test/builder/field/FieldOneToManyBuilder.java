 ...
 *
 */
public class FieldOneToManyBuilder extends
    ...
    public List<ValidationValue<Collection<IEntity<Serializable>>>> getValidationErrorValues(
        ...
        final List<ValidationValue<Collection<IEntity<Serializable>>>> list = ListUtil
                .toList();
        // build bad entities.
        final AbstractEntityBuilder<IEntity<Serializable>> entityBuilder = getFieldBuilderFactory()
                .getEntityBuilderFactory().getBuilder(entityClass);
        final List<ValidationValue<IEntity<Serializable>>> badEntities = entityBuilder
        ...
        for (final ValidationValue<IEntity<Serializable>> badEntity : badEntities) {
            ...
            list.add(new ValidationValue<Collection<IEntity<Serializable>>>(
                    badEntity, subList));
        }
        return list;
    }
    ...
    public void setFieldValue(final Object to, final Field field,
        ...
        final boolean injectParent = !fieldValue.isNullManyToOne()
     ...
     *            the value to set.
     * @param injectParent
     *            if true, the parent object is injected into the child object.
     */
    @SuppressWarnings("unchecked")
    private void setFieldValue(final Object to, final Field field,
            final Object fieldValues, final boolean injectParent) {
        try {
            Assert.assertTrue(CollectionUtil.isCollection(field.getType()));
            Assert.assertTrue(CollectionUtil.isCollection(fieldValues
                    .getClass()));
            final Collection<Object> toValues = ObjectUtil.cast(
                    InvocationUtil.getValue(to, field), Collection.class);
            Assert.assertNotNull(
                    "Null value return by getter on OneToMany field " + field,
                    toValues);
            toValues.clear();
            final Collection<Object> fromValues = ObjectUtil.cast(fieldValues,
                    Collection.class);
     ...
     *
     * @param parent
     *            the parent object to inject.
     * @param to
     *            the object to set the parent.
     * @param toFieldName
     *            the field's name of the to object to set.
     * @throws InvocationException
     *             exception.
     */
    private void injectParent(final Object parent, final Object to,
            final String toFieldName) throws InvocationException {
        // Get field name of to class.
        final Field field = InvocationUtil.getField(to.getClass(), toFieldName);
        Assert.assertNotNull("Cannot find field " + toFieldName + " on object "
                + to, field);
        ...
        if (EntityUtil.isMapsId(field)) {
            ...
            // field.
            injectParent(((IEntity<?>) parent).getPrimaryKey(),
                    EntityUtil.getPrimaryKey((IEntity<?>) to), mapsId);
        }
    }
